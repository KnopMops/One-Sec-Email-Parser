...
def check_mail(mail=''):
	...
	if length == 0:
		...
		print(f'[+] У вас {length} входящих. Почта обнавляется автоматически каждые 5 секунд')
...
def delete_mail(mail=''):
	...
	print('[OFF] Почтовый адрес {mail} был удален\n')
...
def main():
	try:
		...
		print('[OFF] Программа была прервана')
